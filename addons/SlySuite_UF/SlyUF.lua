...
function SlyUF.Disable()
    ...
    ShowBlizzardFrames()
    print("|cff00ccff[SlyUF]|r Disabled â€” default frames restored. /reload to fully unload.")
end
...
ef:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == ADDON_NAME then
        ...
        if SlySuiteDataFrame and SlySuiteDataFrame.Register then
            SlySuiteDataFrame.Register(ADDON_NAME, ADDON_VERSION,
                function()
                    if SlyUF.db.enabled then SlyUF.Enable() end
                end, {
                description = "Unit frames â€” player, target, ToT, party. Replaces ZPerl.",
                slash       = "/slyuf",
                icon        = "Interface\\Icons\\Spell_Nature_NaturesBlessing",
            })
        else
            -- No suite â€” init directly
            if SlyUF.db.enabled then SlyUF.Enable() end
        end
    end
end)
